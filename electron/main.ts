...
ipcMain.handle('excel:import', async (_e, projectData: unknown) => {
  ...
  try {
    // exceljs resolves its own, differently-versioned @types/node Buffer — same name,
    // structurally incompatible declaration. `as any` is the only escape that works
    // here since both are called "Buffer" in this file's scope.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
ipcMain.handle('excel:export-styled', async (_e, projectData: unknown) => {
  ...
  // ── Sheet: Project ──────────────────────────────────────────────────────
  const wsPrj = wb.addWorksheet('Project', { properties: { tabColor: { argb: 'FF185FA5' } } });
  wsPrj.columns = [
  ...
  // ── Sheet: Scenes ────────────────────────────────────────────────────────
  const wsScn = wb.addWorksheet('Scenes', { properties: { tabColor: { argb: 'FF1D9E75' } } });
  ...
  // ── Sheet: Hotspots ──────────────────────────────────────────────────────
  const wsHs = wb.addWorksheet('Hotspots', { properties: { tabColor: { argb: 'FF8B5CF6' } } });
  wsHs.columns = [
  ...
  // ── Sheet: Categories ────────────────────────────────────────────────────
  const wsCat = wb.addWorksheet('Categories', { properties: { tabColor: { argb: 'FFBA7517' } } });
  wsCat.columns = [
    { header: 'slug', key: 'slug', width: 22 },
  ...
  // ── Sheet: Pages ─────────────────────────────────────────────────────────
  const wsPages = wb.addWorksheet('Pages', { properties: { tabColor: { argb: 'FF3B82F6' } } });
  wsPages.columns = [
