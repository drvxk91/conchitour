...
import { checkLicenseStatus, activateLicense, startTrial, deactivateThisMachine, getLocalLicense, } from './license/gate';
...
let initialLicenseStatus = 'none';
// ── Tour preview server ───────────────────────────────────────────────────────
let tourPreviewServer = null;
...
async function startTourPreviewServer(outputDir, defaultLang) {
    if (tourPreviewServer) {
        tourPreviewServer.close();
    ...
    tourPreviewServer = http.createServer(async (req, res) => {
        ...
        if (langMatch)
            filePath = path.join(outputDir, langMatch[1], 'index.html');
        else if (sceneMatch)
            filePath = path.join(outputDir, sceneMatch[2], 'scene', sceneMatch[1], 'index.html');
        else if (pageMatch)
            filePath = path.join(outputDir, 'page', pageMatch[1], pageMatch[2], 'index.html');
        else if (pageNoLang)
            filePath = path.join(outputDir, 'page', pageNoLang[1], defaultLang, 'index.html');
        else
            filePath = path.join(outputDir, pathname.replace(/^\//, ''));
        try {
    ...
    return new Promise((resolve) => {
        tourPreviewServer.listen(0, '127.0.0.1', () => {
            tourPreviewPort = tourPreviewServer.address().port;
...
app.whenReady().then(async () => {
    ...
    initialLicenseStatus = await checkLicenseStatus().catch(() => 'none');
    await createWindow();
    ...
    setInterval(async () => {
        try {
            ...
            if (status === 'expired' || status === 'invalid') {
                BrowserWindow.getAllWindows().forEach((w) => w.webContents.send('license:status-changed', status));
            }
        }
        catch { /* ignore */ }
    }, 24 * 60 * 60 * 1000);
...
ipcMain.handle('excel:backup', async (_e, projectData, projectDir) => {
    try {
        const backupsDir = path.join(projectDir, 'backups');
        await fs.mkdir(backupsDir, { recursive: true });
        const proj = projectData;
        const slug = proj?.meta?.name?.replace(/[^a-z0-9]/gi, '-').toLowerCase() ?? 'project';
        const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
        const filename = `${slug}-backup-${ts}.xlsx`;
        const filePath = path.join(backupsDir, filename);
        const wb = XLSX.utils.book_new();
        const langs = proj.languages?.available ?? ['en'];
        const scenes = proj.scenes ?? [];
        const sceneHeader = ['slug', 'heading', 'gps_lat', 'gps_lng',
            ...langs.flatMap((l) => [`title_${l}`, `description_${l}`, `altText_${l}`]),
        ];
...
// ── Excel export ──────────────────────────────────────────────────────────────
ipcMain.handle('excel:export', async (_e, projectData) => {
    ...
    const scenesHeader = [
        ...
        'category_slugs',
        ...langs.flatMap((l) => [`title_${l}`, `description_${l}`, `alt_text_${l}`]),
    ];
    const scenesRows = (proj.scenes ?? []).map((s) => {
        const catSlugs = (s.categoryIds ?? [])
            .map((id) => (proj.categories ?? []).find((c) => c.id === id)?.slug ?? '')
            .filter(Boolean)
            .join(';');
        return [
            s.id, s.slug,
    ...
    // ── Sheet: Categories ──
    const catHeader = ['slug', 'color', 'icon_svg_path', ...langs.map((l) => `name_${l}`)];
    const catRows = (proj.categories ?? []).map((c) => [
        c.slug, c.color, c.iconSvg ?? '',
        ...langs.map((l) => c.name?.[l] ?? ''),
    ]);
    ...
    const hsHeader = [
        ...
        'url', 'mailto',
        ...langs.flatMap((l) => [`title_${l}`, `label_${l}`, `body_${l}`, `subject_${l}`]),
    ];
    const hsRows = [];
    for (const scene of (proj.scenes ?? [])) {
        for (const hs of (scene.hotspots ?? [])) {
            const targetSlug = hs.type === 'link'
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ? (proj.scenes ?? []).find((s) => s.id === hs.targetSceneId)?.slug ?? ''
                : '';
            hsRows.push([
    ...
    const startSceneSlug = (proj.scenes ?? []).find((s) => s.id === branding.startSceneId)?.slug ?? '';
    const brandingHeader = [
        'primary_color', 'accent_color', 'start_scene_slug',
        ...langs.map((l) => `intro_text_${l}`),
    ];
    const brandingRow = [
        branding.primaryColor ?? '', branding.accentColor ?? '', startSceneSlug,
        ...langs.map((l) => branding.introText?.[l] ?? ''),
    ];
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const pagesExportRows = (proj.pages ?? []).map((p) => [
        p.id, p.slug, p.builtIn ?? '', p.enabled ? 'true' : 'false',
        p.showInFooter ? 'true' : 'false', p.order ?? 0,
        ...langs.map((l) => p.title?.[l] ?? ''),
        ...langs.map((l) => p.content?.[l] ?? ''),
    ]);
    ...
    const gaEventsExport = analytics.events ?? {};
...
ipcMain.handle('excel:download-template', async (_e, projectData) => {
    const result = await dialog.showSaveDialog({
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const proj = projectData;
    const langs = (proj?.languages?.available?.length ? proj.languages.available : ['en']);
    const wb = XLSX.utils.book_new();
    // ── Scenes ──
    const scenesH = [
    ...
    // ── Categories ──
    const catH = ['slug', 'color', 'icon_svg_path', ...langs.map((l) => `name_${l}`)];
    ...
    // ── Branding ──
    const brandH = ['primary_color', 'accent_color', 'start_scene_slug', ...langs.map((l) => `intro_text_${l}`)];
...
function getCellString(cell) {
    ...
    if (typeof v === 'object' && 'formula' in v) {
        const res = v.result;
        if (res === null || res === undefined || res instanceof Error)
            return '';
        return String(res);
    }
...
ipcMain.handle('excel:import', async (_e, projectData) => {
    const result = await dialog.showOpenDialog({
    ...
    if (result.canceled || !result.filePaths[0])
        return { canceled: true, changes: [], validationErrors: [] };
    ...
    try {
        ...
        const err = fsErr;
        if (err.code === 'ENOENT')
            return { canceled: false, changes: [], validationErrors: [], error: 'File not found. It may have been moved or deleted.' };
        if (err.code === 'EBUSY' || err.code === 'EPERM' || err.code === 'EACCES')
            return { canceled: false, changes: [], validationErrors: [], error: 'File is locked by another application. Close it in Excel and try again.' };
    ...
    const proj = projectData;
    const langs = (proj.languages?.available ?? ['en']);
    const defaultLang = proj.languages?.default ?? 'en';
    try {
        const changes = [];
        const validationErrors = [];
        function pushChange(entityType, entityId, entityLabel, field, oldDisplay, newDisplay, patchValue, parentId) {
        ...
        if (scenesWs) {
            ...
            for (const row of dataRows) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const scene = sceneId
                    ? (proj.scenes ?? []).find((s) => s.id === sceneId)
                    : (proj.scenes ?? []).find((s) => s.slug === slugVal);
                if (!scene)
                    continue;
                const label = scene.title?.[defaultLang] ?? scene.title?.en ?? scene.slug;
                ...
                if (headingStr) {
                    ...
                    if (isNaN(headingNum)) {
                        ...
                        pushChange('scene', scene.id, label, 'heading', scene.heading != null ? `${Number(scene.heading).toFixed(1)}°` : '—', `${normalized.toFixed(1)}°`, normalized);
                    }
                }
                ...
                if (capHStr) {
                    ...
                    if (isNaN(capH) || capH <= 0) {
                        ...
                        pushChange('scene', scene.id, label, 'captureHeightMeters', scene.captureHeightMeters != null ? `${scene.captureHeightMeters}m` : '—', `${capH}m`, capH);
                    }
                }
                // Visibility radius
                ...
                if (vrStr) {
                    ...
                    if (isNaN(vr) || vr < 0) {
                        ...
                        pushChange('scene', scene.id, label, 'visibilityRadius', scene.visibilityRadius != null ? String(scene.visibilityRadius) : '—', String(vr), vr);
                    }
                }
                // Category slugs (separator: ; or ,)
                const catSlugsStr = String(row[col('category_slugs')] ?? '').trim();
                if (catSlugsStr) {
                    const slugArr = catSlugsStr.split(/[;,]/).map((s) => s.trim()).filter(Boolean);
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const ids = slugArr.map((sl) => (proj.categories ?? []).find((c) => c.slug === sl)?.id).filter(Boolean);
                    const badSlugs = slugArr.filter((sl) => !(proj.categories ?? []).find((c) => c.slug === sl));
                    ...
                    if (ids.length > 0) {
                        const oldIds = scene.categoryIds ?? [];
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        const oldDisplay = oldIds.map((id) => (proj.categories ?? []).find((c) => c.id === id)?.slug ?? id).join(';') || '—';
                        const newDisplay = ids.map((id) => (proj.categories ?? []).find((c) => c.id === id)?.slug ?? id).join(';');
        ...
        if (catsWs) {
            ...
            for (const row of dataRows) {
                ...
                const slugVal = String(row[col('slug')] ?? '').trim();
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const cat = (proj.categories ?? []).find((c) => c.slug === slugVal);
                if (!cat)
                    continue;
                const label = cat.name?.[defaultLang] ?? cat.name?.en ?? cat.slug;
        ...
        if (pagesWs) {
            ...
            for (const row of dataRows) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const page = idVal
                    ? (proj.pages ?? []).find((p) => p.id === idVal)
                    : (proj.pages ?? []).find((p) => p.slug === slugVal);
                if (!page)
                    continue;
                const label = page.title?.[defaultLang] ?? page.title?.en ?? page.slug;
                ...
                if (enabledStr) {
                    if (enabledStr !== 'true' && enabledStr !== 'false') {
                        ...
                        if (newV !== page.enabled)
                            pushChange('page', page.id, label, 'enabled', String(page.enabled ?? false), enabledStr, newV);
                    }
                }
                ...
                if (footerStr) {
                    if (footerStr !== 'true' && footerStr !== 'false') {
                        ...
                        if (newV !== page.showInFooter)
                            pushChange('page', page.id, label, 'showInFooter', String(page.showInFooter ?? false), footerStr, newV);
                    }
                }
        ...
        if (analyticsWs) {
            ...
            if (headerRow) {
                ...
                function parseAnalyticsBool(key, projKey, val) {
                    const v = val.toLowerCase();
                    if (v !== 'true' && v !== 'false')
                        return;
                    const newV = v === 'true';
                    if (newV !== cur[projKey])
                        pushChange('analytics', 'analytics', 'Analytics', projKey, String(cur[projKey] ?? false), v, newV);
                }
                if (headerRow[0] === 'setting') {
                    ...
                    if (midStr && midStr !== (cur.measurementId ?? '')) {
                        if (!/^G-[A-Z0-9]+$/.test(midStr))
                            validationErrors.push({ entityLabel: 'Analytics', field: 'measurement_id', value: midStr, message: 'Measurement ID should start with "G-" followed by uppercase letters/numbers.' });
                    ...
                    for (const k of evKeys) {
                        ...
                        if (evStr !== 'true' && evStr !== 'false')
                            continue;
                        const newV = evStr === 'true';
                        const oldV = cur.events?.[k] ?? false;
                        if (newV !== oldV)
                            pushChange('analytics', 'analytics', 'Analytics', `events.${k}`, String(oldV), evStr, newV);
                    }
                }
                else {
        ...
        if (projMetaWs) {
            ...
            if (header && dataRow) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const cur = proj.meta ?? {};
                function pushMeta(excelKey, projKey, isUrl = false) {
        ...
        if (modsWs) {
            ...
            if (headerRow?.[0] === 'key') {
                ...
                for (const [excelKey, projKey] of boolModules) {
                    ...
                    const newV = v === 'true';
                    if (newV !== !!cur[projKey])
                        pushChange('modules', 'modules', 'Modules', projKey, String(!!cur[projKey]), v, newV);
                }
                const textModules = [
                ...
                for (const [excelKey, projKey] of textModules) {
                    ...
                    if (val !== old)
                        pushChange('modules', 'modules', 'Modules', projKey, old || '—', val || '(cleared)', val);
                }
            }
        }
        ...
        if (hotspotsWs) {
            ...
            for (const row of dataRows) {
                ...
                if (!hotspotId || !sceneSlugVal)
                    continue;
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const scene = (proj.scenes ?? []).find((s) => s.slug === sceneSlugVal);
                if (!scene)
                    continue;
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...
                if (col('target_scene_slug') >= 0) {
                    ...
                    if (targetSlugStr) {
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        const targetScene = (proj.scenes ?? []).find((s) => s.slug === targetSlugStr);
                        if (!targetScene) {
                            ...
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            const oldSlug = (proj.scenes ?? []).find((s) => s.id === hotspot.targetSceneId)?.slug ?? hotspot.targetSceneId ?? '—';
                        ...
                        // Explicitly cleared
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        const oldSlug = (proj.scenes ?? []).find((s) => s.id === hotspot.targetSceneId)?.slug ?? '—';
...
// ── Git commit ────────────────────────────────────────────────────────────────
ipcMain.handle('project:git-commit', async (_e, projectDir, message) => {
    try {
...
function applyHeaderRow(ws, headerRow) {
...
function decimalValidation(min, max, title, msg) {
    return {
        type: 'decimal',
        operator: 'between',
        allowBlank: true,
...
function slugValidationFor(addr) {
    return {
        type: 'custom',
        allowBlank: true,
...
function colorValidationFor(addr) {
    return {
        type: 'custom',
        allowBlank: true,
...
function urlValidationFor(addr) {
    return {
        type: 'custom',
        allowBlank: true,
...
function noSpacesValidationFor(addr) {
    return {
        type: 'custom',
        allowBlank: true,
...
async function ws_protectKv(ws) {
    await ws.protect('', {
...
async function finalizeSheet(ws, cfg) {
    const colCount = ws.columnCount;
    const rowCount = ws.rowCount;
    for (let r = 2; r <= rowCount; r++) {
        const row = ws.getRow(r);
        for (let c = 1; c <= colCount; c++) {
            const colKey = ws.getColumn(c).key;
...
ipcMain.handle('excel:export-styled', async (_e, projectData) => {
    const result = await dialog.showSaveDialog({
    ...
    for (const sc of scenes) {
        for (const h of (sc.hotspots ?? [])) {
            hotspots.push({ sceneSlug: sc.slug, h });
        }
    }
    const wb = new ExcelJS.Workbook();
    ...
    for (const s of scenes) {
        ...
        for (const l of langs) {
            row[`title_${l}`] = s.title?.[l] ?? '';
            row[`description_${l}`] = s.description?.[l] ?? '';
            row[`altText_${l}`] = s.altText?.[l] ?? '';
        }
    ...
    const targetSceneCfg = sceneSlugFormula && sceneSlugFormula.length <= 253
        ? { list: sceneSlugList }
    ...
    for (const { sceneSlug, h } of hotspots) {
        const row = {
            ...
            ath: h.ath ?? 0,
            atv: h.atv ?? 0,
            target_scene_slug: h.targetSceneId
                ? (scenes.find((s) => s.id === h.targetSceneId)?.slug ?? '')
                : '',
            url: h.url ?? '',
        };
        for (const l of langs) {
            row[`title_${l}`] = h.title?.[l] ?? '';
            row[`body_${l}`] = h.body?.[l] ?? '';
        }
        wsHs.addRow(row);
    }
    ...
    for (const p of (proj.pages ?? [])) {
        ...
        for (const l of langs) {
            row[`title_${l}`] = p.title?.[l] ?? '';
            row[`content_${l}`] = p.content?.[l] ?? '';
        }
        wsPages.addRow(row);
    }
    ...
    for (const [k, v] of modEntries) {
        ...
        if (MOD_BOOL_KEYS.has(k))
            valCell.dataValidation = BOOL_VALIDATION;
    }
    ...
    const anaRows = [
        ...
        ...Object.entries(gaEvents).map(([k, v]) => [`event_${k}`, String(v), true]),
    ];
    ...
    const aiCtx = (proj.aiContext ?? {});
    const AI_CTX_VALIDATIONS = {
    ...
    for (const [setting, value] of aiCtxRows) {
        ...
        const valCell = addedRow.getCell('value');
        valCell.protection = { locked: false, hidden: false };
        if (AI_CTX_VALIDATIONS[setting])
            valCell.dataValidation = AI_CTX_VALIDATIONS[setting];
    }
    ...
    for (const s of scenes)
        wsNotes.addRow({ scene_slug: s.slug ?? '', note: '' });
...
ipcMain.handle('preview:open', async (_e, sourcePath, heading, sceneData) => {
...
ipcMain.handle('project:new', async (_e, parentFolder, projectName) => {
...
ipcMain.handle('project:save', async (_e, projectData) => {
    if (!currentProjectDir)
        throw new Error('No project open — use Save As to create one');
    ...
    try {
        ...
        const existing = JSON.parse(await fs.readFile(lockPath, 'utf-8').catch(() => '{}')).valueOf();
        await fs.writeFile(lockPath, JSON.stringify({ ...existing, lastModified: new Date().toISOString() }, null, 2), 'utf-8');
    }
    catch { /* ignore */ }
    return true;
});
ipcMain.handle('project:save-as', async (_e, projectData) => {
    ...
    if (pick.canceled || !pick.filePaths[0])
        return null;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const projName = projectData?.meta?.name || 'untitled';
    ...
    if (currentProjectDir) {
        for (const sub of ['sources', 'cache', 'assets']) {
            try {
                await copyDir(path.join(currentProjectDir, sub), path.join(destDir, sub));
            }
            catch { /* skip */ }
        }
    }
...
ipcMain.handle('project:copy-source', async (_e, srcPath) => {
    if (!currentProjectDir)
        return null;
...
ipcMain.handle('capture-scene-thumbnail', async (_e, slug, rect) => {
    if (!currentProjectDir)
        return false;
    try {
...
function setupAppMenu() {
    const sendAction = (action) => sendToRenderer(`menu:${action}`);
...
ipcMain.handle('krpano:validate', async (_e, krpanoPath) => {
    const checks = [
    ...
    for (const { file, label } of checks) {
        try {
            await fs.access(path.join(krpanoPath, file));
        }
        catch {
            missing.push(label);
        }
...
ipcMain.handle('krpano:license-status', async (_e, krpanoPath) => {
...
ipcMain.handle('krpano:register', async (_e, krpanoPath, code) => {
    ...
    if (!cleanCode)
        return { ok: false, message: 'Registration code is empty.' };
...
async function hasValidCache(cacheDir, slug, srcHash) {
    try {
...
async function copyCacheTo(cacheDir, slug, destPanosDir) {
    const src = path.join(cacheDir, slug, 'tiles');
    const dest = path.join(destPanosDir, `${slug}.tiles`);
    await copyDir(src, dest);
}
async function buildCacheFor(cacheDir, slug, tilesDir, srcHash) {
    ...
    // Write metadata
    await fs.writeFile(path.join(cacheSlugDir, 'cache.json'), JSON.stringify({ slug, srcHash, cachedAt: new Date().toISOString() }, null, 2), 'utf-8');
}
let compileRunState = null;
let compileCancelToken = { canceled: false };
// ── Preview temp-dir helpers ───────────────────────────────────────────────────
function getTrialPreviewDir() {
    ...
    try {
        ...
        for (const e of entries) {
            if (e.isDirectory() && e.name.startsWith('.conchitour-preview-')) {
                try {
                    await fs.rm(path.join(os.tmpdir(), e.name), { recursive: true, force: true });
                }
                catch { /* ignore */ }
            }
        }
...
function sceneXmlName(slug) {
    return 'scene_' + slug.replace(/-/g, '_');
}
function hotspotXmlName(id) {
    return 'hs_' + id.replace(/-/g, '');
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function loc(record, lang) {
    if (!record)
        return '';
...
async function detectTileInfo(panosDir, slug) {
    const facesDir = path.join(panosDir, `${slug}.tiles`, 'f');
    try {
        await fs.access(facesDir);
    }
    catch {
        return null;
    }
    ...
    for (const levelDir of levelDirs) {
        ...
        try {
            ...
            if (firstRowFiles.length === 0)
                continue;
            // totalWidth = (numCols-1)*tileSize + lastColWidth (edge tile may be smaller)
            const lastColData = await fs.readFile(path.join(facesDir, levelDir, rows[0], firstRowFiles[firstRowFiles.length - 1]));
            ...
            const lastRowFiles = (await fs.readdir(path.join(facesDir, levelDir, rows[rows.length - 1])))
                .filter(f => f.endsWith('.jpg')).sort();
            const lastRowData = await fs.readFile(path.join(facesDir, levelDir, rows[rows.length - 1], lastRowFiles[0]));
...
function geoAth(bearing, heading) {
...
function geoAtv(distM, heightDiffM) {
    if (distM <= 0)
        return 0;
    return -(Math.atan2(heightDiffM, distM) * 180) / Math.PI;
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function generateKrpanoXml(project, tiledScenes) {
    const lang = project.languages?.default || 'en';
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const startSceneId = branding.startSceneId;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const startScene = scenes.find((s) => s.id === startSceneId) ?? scenes[0];
    const startName = startScene ? sceneXmlName(startScene.slug) : '';
    const projectTitle = xmlEsc(project.meta?.name || 'Virtual Tour');
    const useGyro = !!modules.gyroscope;
    const useVr = !!modules.vr;
    ...
    const hsPx = Math.max(16, Math.min(80, branding.hotspotSizePx ?? 32));
    const hsW = Math.round(hsPx * 1.875);
    const hsH = Math.round(hsPx * 2.25);
    ...
    for (const scene of scenes) {
        ...
        const hlookat = dv?.hlookat ?? 0;
        const vlookat = dv?.vlookat ?? 0;
        const fov = dv?.fov ?? 90;
        const heading = scene.heading ?? 0;
        ...
        for (const hs of (scene.hotspots ?? [])) {
            ...
            if (hs.type === 'link') {
                ...
                const ath = hsAth.toFixed(2);
                const atv = hsAtv.toFixed(2);
                // Use target scene's primary category icon as hotspot pin
                const tCatId = targetScene.categoryIds?.[0];
                const tCat = tCatId ? categories.find((c) => c.id === tCatId) : null;
                ...
                const ath = (hs.ath ?? 0).toFixed(2);
                const atv = (hs.atv ?? 0).toFixed(2);
                if (hs.type === 'text') {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function generatePageHtml(project, page, lang, bodyHtml) {
    const meta = project.meta || {};
    const branding = project.branding || {};
    const allLangs = project.languages?.available || [lang];
    const defaultLang = project.languages?.default || 'en';
    const pages = (project.pages || []).filter((p) => p.enabled && p.showInFooter);
    ...
    const fontFamily = tourTheme.fontFamily === 'serif'
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function generateTourHtml(project, lang, startSceneSlug, tiledSlugs, hasSharePreview = false, isTrialBuild = false) {
    const meta = project.meta || {};
    ...
    const categories = project.categories || [];
    const defaultLang = project.languages?.default || 'en';
    const allLangs = project.languages?.available || [defaultLang];
    const startScene = startSceneSlug
        ? scenes.find((s) => s.slug === startSceneSlug)
        : (scenes.find((s) => s.id === branding.startSceneId) ?? scenes[0]);
    ...
    // Footer page links
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const footerPages = (project.pages || [])
        .filter((p) => p.enabled && p.showInFooter)
        .sort((a, b) => (a.order ?? 0) - (b.order ?? 0));
    const tourFooterHtml = footerPages.length > 0
        ? `<div id="tour-footer">${footerPages.map((p, i) => (i > 0 ? '<span class="tf-sep">·</span>' : '') +
            `<a href="/page/${p.slug}/${lang}/">${xmlEsc(loc(p.title, lang) || p.slug)}</a>`).join('')}</div>`
        : '';
    ...
    for (const scene of scenes) {
        ...
        const slugNorm = scene.slug.replace(/[-_]/g, ' ').toLowerCase().trim();
    ...
    const scenesIndexData = {};
    for (const scene of scenes) {
        scenesIndexData[scene.slug] = { title: scene.title || {}, slug: scene.slug };
    }
    const categoriesIndexData = {};
    for (const cat of categories) {
        const rawCatIcon = cat.iconSvg;
        let resolvedCatIcon = null;
        if (rawCatIcon?.startsWith('builtin:')) {
            const inner = BUILTIN_ICON_SVG[rawCatIcon.slice(8)];
            if (inner)
                resolvedCatIcon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${inner}</svg>`;
        }
        else if (rawCatIcon) {
            resolvedCatIcon = rawCatIcon;
        }
        categoriesIndexData[cat.slug] = { name: cat.name || {}, color: cat.color || accentColor, iconSvg: resolvedCatIcon };
    }
    ...
    const sceneCategoriesIndexData = {};
    for (const scene of scenes) {
        const catSlugSet = new Set();
        for (const hs of (scene.hotspots ?? [])) {
            if (hs.type === 'link' && hs.targetSceneId) {
                const tScene = scenes.find((s) => s.id === hs.targetSceneId);
                const tCatId = tScene?.categoryIds?.[0];
                const tCat = tCatId ? categories.find((c) => c.id === tCatId) : null;
                if (tCat?.slug)
                    catSlugSet.add(tCat.slug);
            }
        }
        sceneCategoriesIndexData[scene.slug] = Array.from(catSlugSet);
    }
    ...
    for (const scene of scenes) {
        ...
        for (const hs of (scene.hotspots ?? [])) {
            if (hs.type === 'link' && hs.targetSceneId) {
                const tScene = scenes.find((s) => s.id === hs.targetSceneId);
                if (tScene?.slug)
                    slugMap[tScene.slug] = hotspotXmlName(hs.id);
            }
        }
        sceneHotspotMapData[scene.slug] = slugMap;
    }
    const sceneHotspotMapJson = JSON.stringify(sceneHotspotMapData).replace(/<\//g, '<\\/');
    // Categories keyed by ID
    const categoriesData = {};
    for (const cat of categories) {
        let resolvedIconSvg = null;
        if (cat.iconSvg) {
            const rawIcon = cat.iconSvg;
    ...
    const hasMap = scenes.some((s) => s.geo?.lat != null && s.geo?.lng != null);
    // OG / canonical
    let headExtras = '';
    if (canonicalUrl) {
    ...
    if (description)
        headExtras += `  <meta property="og:description" content="${description}">\n`;
    ...
    if (publicUrl) {
        ...
        if (ogImageUrl) {
            ...
            if (description)
                headExtras += `  <meta name="twitter:description" content="${description}">\n`;
            headExtras += `  <meta name="twitter:image" content="${ogImageUrl}">\n`;
        }
    }
    if (keywords.length)
        headExtras += `  <meta name="keywords" content="${xmlEsc(keywords.join(', '))}">\n`;
    // GA4 analytics
    const _ga = project.analytics ?? {};
    const gaEnabled = !!(_ga.enabled &&
        _ga.measurementId &&
        /^G-[A-Z0-9]{9,12}$/.test(_ga.measurementId));
    const gaMid = _ga.measurementId || '';
    const gaAnonymize = _ga.anonymizeIp !== false;
    const gaConsent = !!_ga.respectCookieConsent;
    const gaEvents = _ga.events ?? {};
    if (gaEnabled) {
    ...
    let shareStyles = '';
    let shareBar = '';
    const hasShare = !!(share.facebook || share.twitter || share.whatsapp || share.linkedin || share.email);
    if (hasShare) {
        shareStyles =
        ...
        const links = [];
        if (share.facebook)
            links.push(`<a href="#" onclick="_track('share_click',{platform:'facebook'});window.open('https://facebook.com/sharer/sharer.php?u='+encodeURIComponent(location.href),'_blank','noopener,noreferrer');return false;" title="Facebook">f</a>`);
        if (share.twitter)
            links.push(`<a href="#" onclick="_track('share_click',{platform:'twitter'});window.open('https://x.com/intent/tweet?url='+encodeURIComponent(location.href)+'&text='+encodeURIComponent(document.title),'_blank','noopener,noreferrer');return false;" title="X">X</a>`);
        if (share.whatsapp)
            links.push(`<a href="#" onclick="_track('share_click',{platform:'whatsapp'});window.open('https://api.whatsapp.com/send?text='+encodeURIComponent(document.title)+'%20'+encodeURIComponent(location.href),'_blank','noopener,noreferrer');return false;" title="WhatsApp">W</a>`);
        if (share.linkedin)
            links.push(`<a href="#" onclick="_track('share_click',{platform:'linkedin'});window.open('https://linkedin.com/sharing/share-offsite/?url='+encodeURIComponent(location.href),'_blank','noopener,noreferrer');return false;" title="LinkedIn">in</a>`);
        if (share.email)
            links.push(`<a href="#" onclick="_track('share_click',{platform:'email'});location.href='mailto:?subject='+encodeURIComponent(document.title)+'&body='+encodeURIComponent(location.href);return false;" title="Email">@</a>`);
    ...
    const loadSceneCall = startSceneSlug
        ...
        : '';
    // ── Loading screen ────────────────────────────────────────────────────
    const loaderExt = branding.loaderPath ? path.extname(branding.loaderPath) : '';
    const loaderImgSrc = branding.loaderPath ? `/assets/loader${xmlEsc(loaderExt)}` : '';
    const introText = xmlEsc(branding.introText?.[lang]
        || branding.introText?.[defaultLang]
        || '');
    const introAnimation = tourTheme.introAnimation || 'fade';
    const introFontSize = Number(tourTheme.introFontSize) || 18;
    const introFontFamily = tourTheme.fontFamily === 'serif'
        ? "Georgia,'Times New Roman',serif"
        : tourTheme.fontFamily === 'mono'
            ? "ui-monospace,'SFMono-Regular',Menlo,monospace"
            : "-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif";
    const splashAnimName = introAnimation === 'slide' ? '_splashSlide' : introAnimation === 'zoom' ? '_splashZoom' : '_splashFade';
    // ── Favicon ───────────────────────────────────────────────────────────
    const faviconExt = branding.faviconPath ? path.extname(branding.faviconPath) : '';
    const faviconLinkHtml = branding.faviconPath ? `  <link rel="icon" href="/assets/favicon${xmlEsc(faviconExt)}">\n` : '';
    // ── Header logo / tour title pill ────────────────────────────────────
    const logoExt = branding.logoPath ? path.extname(branding.logoPath) : '';
    const pillInitial = (projectTitle || '?').trim().charAt(0).toUpperCase();
    // logoPath file > initial letter from tour title
    const logoImgHtml = branding.logoPath
    ...
    const initialSheetTitle = startScene
        ? (loc(startScene.title, lang) || loc(startScene.title, defaultLang) || (Object.values(startScene.title || {})[0] || ''))
        : '';
    const initialSheetCat = (() => {
        if (!startScene)
            return '';
        const catId = (startScene.categoryIds || [])[0];
        if (!catId)
            return '';
        const cat = (project.categories || []).find((c) => c.id === catId);
        return cat ? (loc(cat.name, lang) || loc(cat.name, defaultLang) || (Object.values(cat.name || {})[0] || '')) : '';
    })();
    // ── Header action buttons ─────────────────────────────────────────────
    // Map is shown when scenes have GPS. mapMode config controls appearance only.
    const showMap = hasMap;
    const mobView = branding.mobileDefaultView || 'map';
    const bodyClasses = [
        showMap && mobView === 'map' ? 'has-map' : '',
        mobView === 'strip' ? 'has-strip' : '',
        mobView === 'pano' ? 'pano-only' : '',
    ].filter(Boolean).join(' ');
    const mapHdrBtn = showMap
    ...
    const langHdrBtns = allLangs.length > 1
    ...
    const shareHdrItems = [];
    if (share.facebook)
        shareHdrItems.push(`<a class="shr-pop-item" href="#" onclick="_track('share_click',{platform:'facebook'});window.open('https://facebook.com/sharer/sharer.php?u='+encodeURIComponent(location.href),'_blank','noopener');return false;" title="Facebook">f</a>`);
    if (share.twitter)
        shareHdrItems.push(`<a class="shr-pop-item" href="#" onclick="_track('share_click',{platform:'twitter'});window.open('https://x.com/intent/tweet?url='+encodeURIComponent(location.href)+'&text='+encodeURIComponent(document.title),'_blank','noopener');return false;" title="X">𝕏</a>`);
    if (share.whatsapp)
        shareHdrItems.push(`<a class="shr-pop-item" href="#" onclick="_track('share_click',{platform:'whatsapp'});window.open('https://api.whatsapp.com/send?text='+encodeURIComponent(document.title)+'%20'+encodeURIComponent(location.href),'_blank','noopener');return false;" title="WhatsApp">W</a>`);
    if (share.linkedin)
        shareHdrItems.push(`<a class="shr-pop-item" href="#" onclick="_track('share_click',{platform:'linkedin'});window.open('https://linkedin.com/sharing/share-offsite/?url='+encodeURIComponent(location.href),'_blank','noopener');return false;" title="LinkedIn">in</a>`);
    ...
    const hasHdrShare = shareHdrItems.length > 0; // true whenever any share item exists
    const shareHdrHtml = hasHdrShare
        ...
        : '';
    const sharePopoverHtml = hasHdrShare
        ? `  <div id="share-popover">${shareHdrItems.join('')}</div>\n`
        : '';
    // ── Mobile ⋮ More popover ─────────────────────────────────────────────
    const mobMoreItems = [];
    mobMoreItems.push(`<button class="mob-more-item" onclick="_toggleInfo();_closeMobMore()">` +
    ...
    if (project.modules?.vr) {
        mobMoreItems.push(`<button class="mob-more-item" onclick="if(_krpano)_krpano.call('webvr.enterVR()');_closeMobMore()">` +
    ...
    if (project.modules?.fullscreen !== false) {
        mobMoreItems.push(`<button class="mob-more-item" onclick="_toggleFs();_closeMobMore()">` +
    ...
    const mobMorePopoverHtml = `  <div id="mob-more-popover">${mobMoreItems.join('')}</div>\n`;
    // ── Cookie consent ───────────────────────────────────────────────────
    const modules = project.modules || {};
    const cookieEnabled = !!(modules.cookieConsent);
    const cookieTextStr = cookieEnabled
        ...
        : '';
    const cookieHtml = cookieEnabled
    ...
    const sceneCardsHtml = scenes.map((scene) => {
        const ext = path.extname(scene.media?.sourcePath || '.jpg') || '.jpg';
        const prev = tiledSlugs.has(scene.slug)
            ? `/thumbs/${scene.slug}.jpg`
            : `/media/${scene.slug}${ext}`;
        const rawT = loc(scene.title, lang) || loc(scene.title, defaultLang) || (Object.values(scene.title || {})[0] || '');
        const tNorm = rawT.replace(/[-_]/g, ' ').toLowerCase().trim();
        const sNorm = scene.slug.replace(/[-_]/g, ' ').toLowerCase().trim();
    ...
    const hsStyle = branding.hotspotPreviewStyle || 'card';
...
${showMap ? `  <div id="map-panel">
  ...
  window.__mapTourSync = ${JSON.stringify(!!project.modules?.mapTourSync)};
...
${showMap ? `  <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
  ...
  var _mapTileStyle = '${xmlEsc(modules.mapMode?.tileStyle || 'streets')}';
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function generateSitemap(project) {
    const meta = project.meta || {};
...
function generateHotspotSvg(color, label, iconSvg) {
...
function generateServerJs(project) {
    ...
    const allLangs = project.languages?.available || [defaultLang];
    const scenes = project.scenes || [];
    const startSceneId = project.branding?.startSceneId;
    const startScene = (startSceneId ? scenes.find((s) => s.id === startSceneId) : null) ?? scenes[0];
    const defaultStartSlug = startScene?.slug || '';
    return `'use strict';
/**
...
function generateNpmPackageJson() {
...
function generateBat(krpanoPath) {
    return `@echo off
cd /d "%~dp0"
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function runCompilePipeline(project, outputDir, opts, onProgress, onTileProgress, getIsCanceled) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const scenes = project.scenes || [];
    const settings = await readSettings();
    const forceRegenTiles = opts.forceRegenTiles ?? false;
    const localLicense = await getLocalLicense();
    const isTrialBuild = localLicense?.status === 'trial';
    const progress = onProgress;
    function checkCancel() {
        if (getIsCanceled())
            throw new Error('Compile canceled');
    }
    try {
        ...
        const logoSrcPath = project.branding?.logoPath;
        if (logoSrcPath) {
            try {
        ...
        const loaderSrcPath = project.branding?.loaderPath;
        if (loaderSrcPath) {
            try {
        ...
        const faviconSrcPath = project.branding?.faviconPath;
        if (faviconSrcPath) {
            try {
        ...
        let mediaSkipped = 0;
        for (const scene of scenes) {
            const src = scene.media?.sourcePath;
            if (!src) {
                mediaSkipped++;
                continue;
            }
        ...
        if (settings.useKrpanoTiles) {
            ...
            if (toolOk) {
                ...
                try {
                    ...
                    if (cleaned > 0)
                        progress(`Cleaned ${cleaned} leftover temp folder(s)`, 'info');
                }
                catch { /* ignore */ }
                const tmpBase = path.join(os.tmpdir(), `conchitour-${Date.now()}`);
                await fs.mkdir(tmpBase, { recursive: true });
                const tileableScenes = scenes.filter((s) => s.media?.sourcePath);
                let tileSceneIdx = 0;
                for (const scene of scenes) {
                    const src = scene.media?.sourcePath;
                    if (!src)
                        ...
                        await new Promise((resolve, reject) => {
                            ...
                            proc.on('close', (code) => {
                                ...
                                if (code === 0)
                                    ...
                                    reject(new Error(`krpanotools exited with code ${code}`));
                            });
        ...
        let sharePreviewGenerated = false;
        {
            ...
            const startSc = scenes.find((s) => s.id === br.startSceneId) ?? scenes[0];
            const srcPath = startSc?.media?.sourcePath;
            if (srcPath) {
                try {
        ...
        const langs = project.languages?.available?.length
        ...
        await fs.writeFile(path.join(hotspotsDir, 'hs-video.svg'), generateHotspotSvg('#f59e0b', 'v', '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3" fill="white" stroke="white" stroke-linejoin="round"/></svg>'), 'utf8');
        ...
        for (const cat of (project.categories || [])) {
            ...
            await fs.writeFile(path.join(hotspotsDir, `cat-${cat.slug}.svg`), generateHotspotSvg(cat.color || '#6b7280', label, cat.iconSvg), 'utf8');
        }
        ...
        await fs.mkdir(thumbsDir, { recursive: true });
        {
            ...
            for (const scene of scenes) {
                ...
                if (customSrc) {
                    try {
                        await fs.access(customSrc);
                        await fs.copyFile(customSrc, destPath);
                        usedCustom = true;
                    }
                    catch { /* fall through */ }
                }
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const enabledPages = (project.pages || []).filter((p) => p.enabled);
        const pagesDefaultLang = project.languages?.default || 'en';
        if (enabledPages.length > 0) {
            for (const page of enabledPages) {
                for (const lang of langs) {
                    const dir = path.join(outputDir, 'page', page.slug, lang);
                    await fs.mkdir(dir, { recursive: true });
                    const md = (page.content?.[lang] || page.content?.[pagesDefaultLang] || '');
        ...
        async function countDir(dir) {
            try {
                ...
                for (const e of entries) {
                    if (e.isDirectory()) {
                        await countDir(path.join(dir, e.name));
                    }
                    else {
                        totalFiles++;
                        try {
...
ipcMain.handle('compile:run', async (event, projectData, outputDir) => {
    const localLicense = await getLocalLicense();
    if (localLicense?.status === 'trial') {
        return { ok: false, error: 'TRIAL_BLOCKED' };
    }
    const project = projectData; // eslint-disable-line @typescript-eslint/no-explicit-any
    const forceRegenTiles = projectData?.__forceRegenTiles === true;
    compileRunState = { running: true, log: [], startedAt: Date.now() };
    compileCancelToken = { canceled: false };
    const token = compileCancelToken;
    const onProgress = (msg, status) => {
        const entry = { msg, status };
        if (compileRunState)
            compileRunState.log.push(entry);
        try {
            event.sender.send('compile:progress', { msg, status });
        }
        catch { /* window closed */ }
    };
    const onTileProgress = (data) => {
        try {
            event.sender.send('compile:tile-progress', data);
        }
        catch { /* window closed */ }
    };
    const result = await runCompilePipeline(project, outputDir, { forceRegenTiles }, onProgress, onTileProgress, () => token.canceled);
    if (compileRunState) {
        compileRunState.running = false;
        compileRunState.result = result;
    }
    try {
        event.sender.send('compile:done', result);
    }
    catch { /* window closed */ }
    return result;
});
ipcMain.handle('preview:start', async (event, projectData) => {
    ...
    if (result.ok && result.previewUrl) {
        try {
            await shell.openExternal(result.previewUrl);
        }
        catch { /* ignore */ }
    }
    else if (!result.ok) {
    ...
    try {
        event.sender.send('compile:done', finalResult);
    }
    catch { /* window closed */ }
    return finalResult;
});
ipcMain.handle('preview:stop', async () => {
    if (tourPreviewServer) {
        await new Promise(r => { tourPreviewServer.close(() => r()); }).catch(() => { });
        tourPreviewServer = null;
        tourPreviewPort = 0;
        if (tourPreviewDir && path.basename(tourPreviewDir).startsWith('.conchitour-preview-')) {
            try {
                await fs.rm(tourPreviewDir, { recursive: true, force: true });
            }
            catch { /* ignore */ }
        }
        tourPreviewDir = '';
...
ipcMain.handle('project:default-output-dir', async () => {
    ...
    try {
        ...
        if (lock.lastOutputDir && typeof lock.lastOutputDir === 'string')
            return lock.lastOutputDir;
    }
    catch { /* fall through */ }
...
ipcMain.handle('tour-server:start', async (_e, outputDir, defaultLang) => {
    try {
...
ipcMain.handle('tour-server:status', async () => {
    ...
    const defaultLang = tourPreviewDir
        ? (await fs.readdir(tourPreviewDir).then((entries) => entries.find((e) => /^[a-z]{2}$/.test(e)) ?? 'en').catch(() => 'en'))
        : 'en';
...
// ── Trial IPC ─────────────────────────────────────────────────────────────────
ipcMain.handle('trial:get-state', async (_e, sceneCount, languageCount) => {
